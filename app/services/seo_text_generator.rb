...
class SeoTextGenerator
  ...
  def build_generation_prompt
    ...
    <<~PROMPT
      ...
      - #{@language == 'ua' ? 'ЗАВЕРШЕННЯ ТЕКСТУ: Одне коротке речення без заголовка з фразою "оформіть замовлення онлайн". Приклад: "Купити шини #{@brand} #{@model} #{@size} можна в каталозі та оформіть замовлення онлайн."' : 'ЗАВЕРШЕНИЕ ТЕКСТА: Одно краткое предложение без заголовка с фразой "оформите заказ онлайн". Пример: "Купить шины #{@brand} #{@model} #{@size} можно в каталоге и оформите заказ онлайн."'}
