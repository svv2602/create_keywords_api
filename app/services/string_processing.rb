...
module StringProcessing
  ...
  def seo_phrase_sentence(element_array, number_of_repeats, ind)
    ...
    if ind > 0
      ...
      topics += "\n "
      topics += "\n ВАЖНО: Если в тексте встречаются ссылки на размеры шин или дисков, ОБЯЗАТЕЛЬНО сохраняй ТОЧНЫЙ формат URL с префиксами w-, h-, r-"
      topics += "\n Пример правильного формата ссылки: /shiny/w-255/h-45/r-19/ или /shiny/letnie/w-175/h-70/r-13/"
      topics += "\n НЕПРАВИЛЬНО: /shiny/255/45/r19/ или /shiny/w255/h45/r-19/"
      topics += "\n ПРАВИЛЬНО: /shiny/w-255/h-45/r-19/"
      topics += "\n Всегда используй дефис после префиксов w-, h-, r- и слэш между параметрами размера."
      topics += "\n "
