  ...
  def build_russian_prompt
    ...
    prompt = <<~PROMPT
      ...
      6. <h3>Преимущества ProKoleso</h3>
         <p>Краткое описание преимуществ (2-3 предложения): широкий выбор, проверенные бренды, доставка, консультация.
         ОБЯЗАТЕЛЬНО завершить текст фразой "оформите заказ онлайн" (именно эти слова!).</p>
  ...
  def build_ukrainian_prompt
    ...
    prompt = <<~PROMPT
      ...
      6. <h3>Переваги ProKoleso</h3>
         <p>Короткий опис переваг (2-3 речення): широкий вибір, перевірені бренди, доставка, консультація.
         ОБОВ'ЯЗКОВО завершити текст фразою "оформіть замовлення онлайн" (саме ці слова!).</p>
