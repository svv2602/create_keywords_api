  ...
  def build_russian_prompt
    ...
    prompt = <<~PROMPT
      ...
      #{@wikipedia_info.present? ? "3. <h3>Об автомобиле #{@brand.capitalize} #{@model.capitalize}</h3>
         <p>2-3 предложения об автомобиле на основе информации из Wikipedia:
         - Годы выпуска, поколения
         - Класс автомобиля, позиционирование
         - Ключевые особенности модели (двигатели, платформа, популярность)
         Используй ТОЛЬКО факты из предоставленной информации Wikipedia, не выдумывай!</p>
  ...
  def build_ukrainian_prompt
    ...
    prompt = <<~PROMPT
      ...
      #{@wikipedia_info.present? ? "3. <h3>Про автомобіль #{@brand.capitalize} #{@model.capitalize}</h3>
         <p>2-3 речення про автомобіль на основі інформації з Wikipedia:
         - Роки випуску, покоління
         - Клас автомобіля, позиціонування
         - Ключові особливості моделі (двигуни, платформа, популярність)
         Використовуй ТІЛЬКИ факти з наданої інформації Wikipedia, не вигадуй!</p>
