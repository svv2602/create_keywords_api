...
class CarSeoTextGenerator
  ...
  def build_russian_prompt
    ...
    prompt = <<~PROMPT
      ...
      Пример: "Купить шины на #{@brand.capitalize} #{@model.capitalize} можно в каталоге и оформите заказ онлайн."
      НЕ повторяй информацию из статьи, НЕ добавляй расширенные пояснения.
  ...
  def build_ukrainian_prompt
    ...
    prompt = <<~PROMPT
      ...
      Приклад: "Купити шини на #{@brand.capitalize} #{@model.capitalize} можна в каталозі та оформіть замовлення онлайн."
      НЕ повторюй інформацію зі статті, НЕ додавай розширені пояснення.
